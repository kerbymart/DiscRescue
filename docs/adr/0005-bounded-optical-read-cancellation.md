...
Native adapters wrap their raw source in `recovery.ReopenableReaderAt`. On
context cancellation it closes the active source, waits for the request to
finish, reopens the same source, and only then returns the timeout to the
scheduler. This preserves serialized device access and makes later LBAs safe
to read. A source that cannot implement this cancellation boundary must not
be used as a native optical adapter.
...
- Stop and force-stop can close the same serialized source boundary without
  leaving a blocked read running concurrently with a new request.
