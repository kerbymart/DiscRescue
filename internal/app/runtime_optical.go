...
func (m ProgramModel) runOpticalEffect(request EffectRequestedMsg) tea.Msg {
	...
	case EffectDiscoverDevices:
		drives, err := m.services.Optical.DiscoverOpticalDrives()
	...
	case EffectIdentifyMedia:
		media, err := m.services.Optical.IdentifyOpticalMedia(request.DevicePath)
		if err != nil {
	...
	case EffectEject:
		ejector, ok := m.services.Optical.(OpticalEjector)
		if !ok {
